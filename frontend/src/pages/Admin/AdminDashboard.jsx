      ...
      <div className="relative">
        ...
        <div className="relative bg-gradient-to-br from-white/10 via-white/5 to-transparent backdrop-blur-xl border border-white/20 rounded-2xl p-6 shadow-xl">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h3 className="text-2xl font-bold text-white mb-2">Recent Batch Details</h3>
              <p className="text-gray-400">Monitor and manage your latest batch operations</p>
            </div>
            <div className="flex items-center gap-3">
              <button 
              ...
              <button 
                onClick={() => showToast('Advanced filters coming soon!', 'info')}
                className="px-4 py-2 bg-primary-500/20 hover:bg-primary-500/30 border border-primary-500/30 rounded-xl text-primary-300 transition-all duration-200 flex items-center gap-2"
              >
                <Filter className="w-4 h-4" />
                Filter
              </button>
              <button 
                onClick={() => showToast('Viewing all batches...', 'info')}
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-white/5 border-b border-white/10">
                    ...
                    <th className="text-left py-4 px-6 text-gray-300 font-semibold">Quality</th>
                  ...
                  {batches.map((batch, index) => (
                    ...
                    >
                      <td className="py-4 px-6">
                        <span className="font-mono text-primary-400 font-semibold">{batch.id}</span>
                      </td>
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 bg-gradient-to-br from-emerald-500 to-primary-500 rounded-lg flex items-center justify-center">
                      ...
                      <td className="py-4 px-6">
                        <span className="text-gray-400">{batch.location}</span>
                      </td>
                      <td className="py-4 px-6">
                        <StatusBadge status={batch.status} />
                      </td>
                          ...
                          <div className="w-16 h-2 bg-gray-700 rounded-full overflow-hidden">
                            <div 
                              className={`h-full rounded-full ${
                                batch.qualityScore >= 90 ? 'bg-gradient-to-r from-emerald-500 to-primary-500' : 
                                batch.qualityScore >= 70 ? 'bg-gradient-to-r from-yellow-500 to-orange-500' : 
                                'bg-gradient-to-r from-red-500 to-pink-500'
                              }`}
                              style={{ width: `${batch.qualityScore}%` }}
                            />
                          </div>
                          <span className="text-white font-semibold text-sm">{batch.qualityScore}%</span>
                        </div>
                      </td>
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                          <button 
                            onClick={(e) => {
                              e.stopPropagation();
                              showToast(`Viewing ${batch.id}`, 'info');
                            }}
                          ...
                          <button 
                            onClick={(e) => {
                              e.stopPropagation();
                              showToast(`More options for ${batch.id}`, 'info');
                            }}
                            className="p-2 bg-white/10 hover:bg-white/20 rounded-lg transition-all duration-200"
                            title="More Options"
                          >
          ...
          <div className="flex items-center justify-between mt-6 pt-6 border-t border-white/10">
            <p className="text-gray-400 text-sm">
              Showing {batches.length} of {stats?.totalCollections || 1547} total batches
            </p>
            <div className="flex items-center gap-2">
